...
def timeback(start_time,img_root,x1,y1,x2,y2,thred1,thred2,distance):
    ...
    for t in range(int(float(start_time) * 30), max(5,int(float(start_time-15) * 30)), -5):
        dis_list = []
        dis_ = []
        ...
        if (bt - bt_all) < 100 and np.sum(dis_) > 4:
            print("jump from judge1 |Bright:{0} Num:{1} Score:{x}".format(bt-bt_all,np.sum(dis_),x=dis_list))
            return str(t / 30)
        if (bt - bt_all) >= 100 and float(np.mean(dis_list)) <= thred2:
            print("jump from judge2 |Bright:{0} Score:{1}".format(bt-bt_all,np.mean(dis_list)))
            return str(t / 30)

    return max(0,start_time-15)

video_name_start = int(sys.argv[1])
video_name_end=int(sys.argv[2])

psnr_thred=25
psnr_avg_thred=20

euclidean_thred=0.7
euclidean_avg_thred=0.65

ssim_thred=0.45
ssim_avg_thred=0.5

#time back tracing for each frame
...
output="./time_back_test_"+type_str[type_num]+"_"+str(thred[type_num][0])+"_"+str(thred[type_num][1])+'/'
...
for video_name in range(video_name_start,video_name_end+1):
    ...
    try:
        line = open("../../../coarse_filter/"+ str(video_name) + "_coarse_box.txt","r")
        ...
        for key in video_result:
            ...
            #smaller bbox with softer constraint
            if w<=4 or h <=4:
                thred[type_num][0] = max(thred[type_num][0] * 0.2,18)
                thred[type_num][1] = thred[type_num][1] * 0.4
            if w>=50 or h>=50:
                thred[type_num][0] = max(thred[type_num][0] * 1.2, 25)
                thred[type_num][1] = thred[type_num][1] * 1.2
            print(thred[type_num])
